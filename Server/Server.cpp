...
void AServer::TCPSocketListener()
{
    ...
    FString sendText("HELLO CLIENT FROM UNREAL!"); //Send message back

    TArray<uint8> SendBuff = ConvertToUTF8(sendText);
    int32 SendReader;

    ConnectionSocket->Send(SendBuff.GetData(), SendBuff.Num(), SendReader);
}
